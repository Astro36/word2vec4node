...
class Word2Vec {
  ...
  static load(file) {
    if (fs.existsSync(file)) {
      if (file.endsWith('.json')) {
        return Word2Vec.loadJSON(file);
      } else if (file.endsWith('.txt')) {
        return Word2Vec.loadText(file);
      }
    }
    return null;
  }
  ...
  static loadJSON(file) {
    ...
    return Word2Vec.create(vectors);
  }
   ...
   * @returns {Array.<{text: string, vector: Vector}>}
   */
  findDifference(texts, amount = 1) {
    const { size } = this;
    const inputVectors = texts.map(value => this.getVector(value) || Vector.zeros(size));
    const meanVector = Vector.mean(inputVectors);
    return inputVectors.map((value, index) => [
      texts[index],
      value,
